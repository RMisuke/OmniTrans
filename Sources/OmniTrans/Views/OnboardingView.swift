...
/// - The main container uses `.glassCardStyle()` for a unified adaptive
///   glass appearance matching the floating translation panel.
/// - Navigation buttons use subtle material backgrounds (`.regularMaterial`
///   for Previous, `.borderedProminent` for Next/Start) instead of
///   generic hardcoded solid colours.
/// - Page transitions are gated by `AnimationGate` — when animations
///   are globally disabled, pages swap instantly with zero transition
///   overhead.
struct OnboardingView: View {
    ...
    private var permissionsPage: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                OnboardRow(
                    icon: "hand.tap.fill", color: .blue,
                    title: "辅助功能权限",
                    desc: "用于读取划词选中的文本内容。系统弹窗已自动打开，请在隐私设置中勾选 OmniTrans"
                )
                OnboardRow(
                    icon: "rectangle.inset.filled", color: .orange,
                    title: "屏幕录制权限",
    ...
    private var offlineTranslationPage: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                OnboardRow(
                    icon: "apple.logo", color: .blue,
                    title: "完全离线 · 零延迟",
                    desc: "基于 Apple 芯片 ANE 神经网络引擎，翻译不消耗大模型 Token，无网络流量"
                )
                OnboardRow(
                    icon: "arrow.down.to.line.compact", color: .orange,
                    title: "首次使用需下载语言包",
    ...
    private var featuresPage: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                OnboardRow(
                    icon: "rectangle.expand.vertical", color: .indigo,
                    title: "动态窗口 + 场景预设",
                    desc: "小 / 默认 / 大 / 动态 四种尺寸；翻译/润色/口语/代码/文案 5 种 Prompt 模板"
                )
                OnboardRow(
                    icon: "gearshape.fill", color: .secondary,
                    title: "灵活配置",
